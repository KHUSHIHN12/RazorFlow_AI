...
export default function AIGrowthActions({ actions = [] }) {
  const hasNoData = !actions || actions.length === 0 || actions[0]?.insight === "No sufficient data yet";
    ...
    <div className="space-y-3">
      ...
      {hasNoData ? (
        <div className="p-8 rounded-2xl bg-white border border-slate-200 text-center space-y-2">
          ...
          <h3 className="text-sm font-bold text-slate-700">No sufficient data yet</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            Interact with the AI shopping agent to generate data-driven pattern analysis and actionable recommendations.
          </p>
        </div>
      ) : (
